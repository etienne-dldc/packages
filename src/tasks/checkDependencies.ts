...
import { PackageJsonKey } from "./readPackageJson.ts";

export const DevDepsKey = createKey<Record<string, string>>("DevDeps");

export function checkDependencies(pkg: PkgStack): PkgStack {
  ...
  if (missingDevDeps.length > 0) {
    logger.log(
      `${pc.red("◆")} Missing devDependencies: ${missingDevDeps.join(", ")}`
    );
